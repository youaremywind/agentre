...
const backendHints: Record<string, string> = {
      ...
      <header className="space-y-3 border-b border-border bg-card px-5 py-4">
        ...
        {reportTarget && (
          <div className="flex items-center gap-1.5 text-2xs text-muted-foreground">
            ...
            <span className="opacity-60">· 拖拽卡片可调整层级</span>
          </div>
        )}
      </header>
      ...
      <div className="flex-1 space-y-6 overflow-y-auto px-5 py-5">
        <section className="space-y-4" data-slot="agent-section-basic">
          ...
          <div className="space-y-2">
            <label className="block text-2xs text-muted-foreground">头像</label>
            <div className="flex items-center gap-3">
              ...
              <AgentAvatarUploadActions
                ...
                uploadLabel={
                  props.agent.avatarDataUrl ? "替换图片" : "上传图片"
                }
              />
            </div>
          </div>
        ...
        <section className="space-y-2.5" data-slot="agent-section-backend">
                ...
                {selectedBackend ? (
                  <div className="flex min-w-0 flex-col items-start">
                    ...
                    <span className="max-w-full truncate font-mono text-2xs font-normal text-muted-foreground">
                      {backendProviderSummary(selectedBackend)}
                    </span>
                  </div>
                ) : (
          ...
          <p className="text-2xs text-muted-foreground">
            {backendHints[selectedBackend?.type ?? ""] ??
              "选择一个 Agent 后端来执行任务。"}
          </p>
        </section>
        ...
        <section className="space-y-2" data-slot="agent-section-prompt">
          ...
          <div className="flex items-center gap-1.5 text-2xs text-muted-foreground">
            ...
            <span>风格 / 语气 / 边界都写进 system prompt — 不另设字段。</span>
          </div>
        </section>
        ...
        <section className="space-y-2.5" data-slot="agent-section-skills">
          ...
          {skills.length === 0 ? (
            <p className="text-2xs text-muted-foreground">
              暂无技能。可在 Update 时通过设置 skills 添加。
            </p>
          ) : (
            <div
      ...
      <footer className="flex items-center gap-2 border-t border-border bg-secondary/40 px-5 py-3">
        <span className="flex flex-1 items-center gap-1.5 font-mono text-2xs text-muted-foreground">
          ...
          {props.agent.updatetime > 0
            ? `已保存 · ${formatRelativeTime(props.agent.updatetime)}`
            : "未保存"}
        </span>
            ...
            <DialogHeader>
              <DialogTitle className="flex items-center gap-2">
                <AlertTriangle
                  ...
                  aria-hidden="true"
                />
                <span>确认删除 Agent「{props.agent.name}」？</span>
              </DialogTitle>
              <DialogDescription>
                该 Agent 的对话、技能与状态记录都会一并清除，该操作不可撤销。
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
...
function backendProviderSummary(backend: BackendSummaryLike): string {
  const providerName = backend.llmProviderName?.trim();
  const model = backend.llmProviderModel?.trim();
  const inactiveSuffix =
    backend.llmProviderActive === false && providerName ? "（已停用）" : "";
