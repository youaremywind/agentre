...
// xterm 用全局 _keyDownSeen 给 IME composed insertText 做去重, 假定一次只按一个
// 键。百度五笔等输入法在「英文模式」下把每个按键都伪装成 keyCode=229, 加上用户
// 快速输入造成 key-rollover (前一键 keyup 之前下一键 input 已触发), xterm 误判
// 「_keyDownSeen=true => 重复输入」把中间字符丢弃 —— 表现为快速输入 123 丢成 1 / 13。
//
// 旁路逻辑: 监听 textarea 的 input 事件, 在 xterm 跳过且 keypress 路径没处理过的
// 场景下补一次 write。条件必须严于 xterm 的跳过条件, 否则会把"xterm 通过 keypress
// 路径正常发送的字符"(空格 keyCode=32, 大写字母走 Caps Lock HACK) 当成漏字符再补
// 一次, 造成双发。
//
// 参考: node_modules/@xterm/xterm/src/browser/CoreBrowserTerminal.ts (6.0.0)
...
export function attachXtermRolloverGuard(
  term: XTerminal,
  ...
  const internals = (term as unknown as { _core?: XTermRolloverInternals })
    ._core;
