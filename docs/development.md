...
SOLID 落到这个仓库的具体形态 —— 提 PR 前对照检查：

**高内聚（聚在一起的理由要单一）**

- 一个 domain 一套包：`model/entity/<domain>_entity` + `repository/<domain>_repo` + `service/<domain>_svc`。新 domain 开新包，**不要**把功能塞进沾边的旧包凑数。
- 充血实体扛单实体逻辑（`Check(ctx)` / `IsActive()` / `GetXxx/SetXxx`）；service 只做跨实体协调和外部依赖编排。规则下沉到 entity，service 才不会膨胀。
- Wails 绑定一个 domain 一个文件（`internal/app/<domain>.go`），方法只 parse → `svc.Xxx().Method` → return。
- 横切能力（`diff` / `jsonrpc` / `keychain` / `paths` / `remotefs` …）放 `internal/pkg/<concern>`，每个包单一关注点、self-contained；别把它们散进某个 domain。

**低耦合（跨边界只认接口）**

- 依赖单向：`internal/app → service → repository → model/entity`。`internal/pkg` 是叶子横切层，**被各层引用但绝不反向 import** `service` / `repository`（当前确实零反向依赖，守住它）。
- DIP：`service` 只依赖 `repository` **接口**，具体实现通过 `RegisterXxx(impl)` 在 bootstrap/main 装配；这正是 service 单测能用 mockgen 注入的前提。
- ISP：接口在**消费方**定义、按需收窄；消费方要 2 个方法别去依赖 20 个方法的胖接口。
- 跨包协作走 accessor（`xxx_repo.Xxx()` 拿仓储、`xxx_svc.Default()` 拿别的 service），不要 `new` 别人的实现，也不要直接 `db.Ctx` 摸别的 domain 的表。
- 不越级：`internal/app` 不直接碰 `repository` / `db`（绑定层只认 service），service 不绕过自己的 repo 拼裸 SQL。
- 边界封装：前后端只走 Wails binding（不加 HTTP-style app API）；远端执行细节锁在 `internal/daemon/client` 接口后，上层 `chat_svc` 不感知子进程 / 连接细节。
