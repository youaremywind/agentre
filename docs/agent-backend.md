...
| `CapMCPTools` / `"mcp_tools"` | `RunRequest.MCPServers` (no sub-interface) | ❌ | ✅ | ❌ | ❌ | host→backend **launch-time MCP injection**: the runtime accepts `RunRequest.MCPServers` (`[]MCPServerSpec`: `Name` / `URL` / `Headers`, http transport) and starts the turn with those extra MCP tool servers wired in. claudecode renders each spec into a `--mcp-config` entry (`{"mcpServers":{"<name>":{"type":"http","url":"...","headers":{...}}}}`) and auto-adds `mcp__<Name>__group_send` to `--allowedTools` (`runtimes/claudecode/session.go` `buildMcpConfigJSON` — the allowed-tool name is currently hard-coded to `group_send`, the only injected tool). Group-chat orchestration is the first/only consumer: a member backend **must** declare this cap to be admitted to a group (`group_svc.backendSupportsGroup` → `chat_svc.AgentBackendHasCapability`); builtin / codex / piagent ignore `RunRequest.MCPServers` |
